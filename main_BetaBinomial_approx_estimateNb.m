function void = main_BetaBinomial_approx_estimateNb(void)
...
outfile = strcat(sim_data_results.infile, '_betabinomial_approx');
...
for site = 1:sim_data.n_variants
    ...
    sim_data_results.log_likelihood_matrix(site,:) = GetBetaBinomialApproxLogLikelihoodAtSite(sim_data.donor_freqs_observed(site), sim_data.recipient_freqs_observed(site), sim_data_results.Nb_vals, sim_data.var_calling_threshold);
    if mod(site, 50) == 0
